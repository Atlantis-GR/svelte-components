...
import { ApiResponse, PaginatedResponse, Post, CreatePostRequest, UpdatePostRequest } from '../types/index.js';
...
// Create new post
router.post('/', authenticateToken, logActivity('create', 'post'), async (req: Request, res: Response) => {
  try {
...
// Update post
router.put('/:id', authenticateToken, logActivity('update', 'post'), async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    ...
    const isPublished = updateData.status ? (updateData.status === 'published' ? 1 : 0) : existingPost.is_published;
...
// Delete post
router.delete('/:id', authenticateToken, logActivity('delete', 'post'), async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
