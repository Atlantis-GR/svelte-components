...
export const apiClient = {
  ...
  async createConfig(config: { key: string; value: any; description?: string; isSecure?: boolean }) {
    return apiRequest<ConfigurationItem>('/config', {
      method: 'POST',
  ...
  async getPublicPosts(page = 1, limit = 10): Promise<PaginatedResponse<Post>> {
    return apiRequest<Post[]>(`/posts/public?page=${page}&limit=${limit}`) as Promise<PaginatedResponse<Post>>;
  },

  async getAllPosts(page = 1, limit = 10): Promise<PaginatedResponse<Post>> {
    return apiRequest<Post[]>(`/posts?page=${page}&limit=${limit}`, {}, true) as Promise<PaginatedResponse<Post>>;
  },
  ...
  async updatePost(id: string, updates: { title?: string; content?: string; status?: 'draft' | 'published' | 'archived' }) {
  ...
  async validateToken() {
    return apiRequest<{ valid: boolean; user: UserProfile }>('/auth/validate', {
      method: 'POST'
    }, true);
  }
};
