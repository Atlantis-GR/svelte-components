<script lang="ts">
	...
	function addCustomSetting({ key, value, type }: { key?: string; value?: string; type?: string } = {}) {
		if (!key || !value) return;
	...
	function getValuePlaceholder(type: string): string {
		switch (type) {
			case 'number': return 'e.g., 42 or 3.14';
			case 'boolean': return 'true or false';
			case 'json': return 'e.g., {"key": "value"}';
			case 'string':
			default: return 'Enter value here';
		}
	}
...
<div class="space-y-8">
	<div class="bg-white shadow rounded-lg p-6">
		<h1 class="text-3xl font-bold text-gray-900 mb-4">Configuration Demo</h1>
		<p class="text-gray-600">
			This page demonstrates the Svelte.Config library features including reactive configuration access,
			feature flags, and integration with Svelte.Auth.
		</p>
	</div>

	<!-- Logger Configuration Info -->
	<div class="bg-gradient-to-r from-blue-50 to-indigo-50 shadow rounded-lg p-6 border border-blue-200">
		<div class="flex items-start space-x-3">
			...
			<div>
				...
				<p class="mt-1 text-blue-700">
					The Config package now uses the <code class="bg-blue-100 px-1 rounded">ILogger</code> interface from 
					<code class="bg-blue-100 px-1 rounded">@atlantis-gr/svelte-logging-abstractions</code> instead of direct console logs.
					This provides better logging control and consistency across the application. The Config package now logs with 
					proper prefixes like <code class="bg-blue-100 px-1 rounded">[Config] [ConfigService]</code> for better debugging.
				</p>
				<p class="mt-2 text-sm text-blue-600">
					Try the actions below and check your browser console to see structured Config package logs!
				</p>
			</div>
		</div>
	</div>
	...
	<div class="bg-white shadow rounded-lg p-6">
		<h2 class="text-2xl font-bold text-gray-900 mb-4">Configuration Overview</h2>
		<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
			...
			<div class="bg-indigo-50 p-4 rounded-lg">
				...
				<p class="text-indigo-700 text-sm break-all">
			...
			<div class="flex space-x-4">
				...
				<div class="space-y-2">
					<div class="flex space-x-2">
						...
						/>
						<select bind:value={typeInput} class="border p-2 rounded">
					...
					<div class="flex space-x-2">
						...
						<button 
							...
							onclick={() => addCustomSetting({ key: keyInput, value: valueInput, type: typeInput })}
	...
	<!-- Custom App Settings -->
	<div class="bg-white shadow rounded-lg p-6">
		<h2 class="text-2xl font-bold text-gray-900 mb-4">Custom App Settings</h2>
						...
						<div class="flex-1">
							...
							<span class="ml-2 text-gray-700">
								{#if typeof value === 'object'}
									<code class="bg-gray-200 px-1 rounded text-sm">{JSON.stringify(value)}</code>
								{:else}
									<code class="bg-gray-200 px-1 rounded text-sm">{String(value)}</code>
								{/if}
		...
		{:else}
			<p class="text-gray-500 italic">No custom settings added yet. Use the form above to add some!</p>
		{/if}
	</div>

	<!-- Periodic Reload Configuration -->
	<div class="bg-white shadow rounded-lg p-6">
		<h2 class="text-2xl font-bold text-gray-900 mb-4">Periodic Configuration Reload</h2>
		<div class="space-y-4">
			<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
				<div class="space-y-2">
					<h3 class="font-semibold text-gray-900">Status</h3>
					<div class="flex items-center space-x-2">
						<div class="w-3 h-3 rounded-full {isPeriodicActive ? 'bg-green-500' : 'bg-red-500'}"></div>
			...
			<div class="bg-blue-50 border border-blue-200 rounded p-4">
				<h4 class="font-medium text-blue-900 mb-2">💡 About Periodic Reload</h4>
	...
	<div class="bg-white shadow rounded-lg p-6">
		<h2 class="text-2xl font-bold text-gray-900 mb-4">Branding Configuration</h2>
		<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
			<div>
				<h3 class="font-semibold text-gray-900 mb-2">Colors</h3>
				<div class="space-y-2">
					<div class="flex items-center space-x-2">
						<div 
	...
	<div class="bg-white shadow rounded-lg p-6">
		<h2 class="text-2xl font-bold text-gray-900 mb-4">Advanced Configuration</h2>
		<div class="space-y-4">
			<div>
				<h3 class="font-semibold text-gray-900 mb-2">Nested Values</h3>
			...
			<div>
				<h3 class="font-semibold text-gray-900 mb-2">Feature Flags Status</h3>
				<div class="space-y-1">
					{#each Object.entries($featureFlags) as [flag, enabled]}
						<div class="flex items-center space-x-2">
							<span class="w-3 h-3 rounded-full {enabled ? 'bg-green-500' : 'bg-red-500'}"></span>
	...
	<FeatureFlag flag="enableConfigDebugger">
		<div class="bg-white shadow rounded-lg p-6">
			<h2 class="text-2xl font-bold text-gray-900 mb-4">Configuration Debugger</h2>
			<ConfigDebugger />
		</div>
	</FeatureFlag>
</div>