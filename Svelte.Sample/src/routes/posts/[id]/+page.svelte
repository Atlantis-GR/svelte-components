...
<div class="mx-auto max-w-4xl">
	...
	{#if error}
		<div class="mb-6 rounded-md border border-red-200 bg-red-50 p-4">
			...
			<div class="mt-4">
				<button onclick={backToPosts} class="text-sm font-medium text-red-600 hover:text-red-800">
					← Back to Posts
				</button>
			</div>
		</div>
				...
				<div class="flex items-start justify-between">
					...
					<div class="ml-4 flex items-center space-x-2">
						...
						{#if canEditPost()}
							<button
								onclick={editPost}
								class="text-sm font-medium text-gray-600 hover:text-yellow-600"
								title="Edit post"
							>
								Edit
							</button>
		...
		<div class="mt-6 flex justify-between">
			...
			{#if canEditPost()}
				<div class="flex space-x-3">
					<button
						onclick={editPost}
						class="inline-flex items-center rounded-md border border-transparent bg-yellow-600 px-4 py-2 text-sm font-medium text-white hover:bg-yellow-700 focus:ring-2 focus:ring-yellow-500 focus:ring-offset-2 focus:outline-none"
					>
						<svg class="mr-2 h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
							<path
								stroke-linecap="round"
